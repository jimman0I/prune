...
export default function BugReportModal({ onClose }) {
  ...
  return (
    <ModalOverlay label={t('bugReport.title')} onClose={onClose} dismissible={state !== 'opening'}>
        ...
        {state === 'opened' ? (
          <>
            ...
            <div className="flex justify-end">
              <button type="button" className="btn-primary px-3 py-1.5 rounded-md text-[12.5px] min-h-[24px]" onClick={onClose}>
                {t('bugReport.close')}
              </button>
            </div>
          </>
