...
export default function LeftoverReview({ scanResult, selected, onToggle, onConfirm, onSkip }) {
  ...
  if (totalItems === 0) {
    return (
      <div className="text-center py-6">
        <p className="text-[13px] text-[#a1a1aa] mb-4">No leftovers found — clean uninstall.</p>
  ...
  return (
    <div>
      <div className="flex items-start gap-2.5 mb-5 px-3.5 py-3 rounded-xl bg-[#f59e0b]/10 border border-[#f59e0b]/25">
        <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" className="text-[#f59e0b] mt-0.5 shrink-0">
          <circle cx="12" cy="12" r="10"></circle>
          <line x1="12" y1="8" x2="12" y2="12"></line>
          <line x1="12" y1="16" x2="12.01" y2="16"></line>
        </svg>
        <div className="text-[12.5px] text-[#fcd34d] leading-relaxed">
      ...
      {failedGroups.map(({ key, label }) => (
        <p key={key} className="text-[12px] text-[#71717a] mb-2">Couldn't check {label.toLowerCase()}.</p>
      ))}
      ...
      <div className="space-y-3">
        {groupsWithItems.map(({ key, label, group }) => {
          ...
          return (
            <div key={key} className="rounded-xl border border-[#27272a] overflow-hidden bg-[#0c0c0e]">
              <button
              ...
              >
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className={`text-[#71717a] transition-transform ${open ? 'rotate-90' : ''}`}>
                  <polyline points="9 18 15 12 9 6"></polyline>
                </svg>
                <span className="text-[13px] font-medium">{label}</span>
                <span className="text-[11px] text-[#71717a] font-mono">({group.items.length})</span>
              </button>
              {open && (
                <div className="divide-y divide-[#18181b] border-t border-[#18181b]">
                    ...
                    return (
                      <label key={itemKey} className="flex items-center gap-3 px-4 py-2.5 cursor-pointer hover:bg-white/[0.02] transition">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="font-mono text-[11.5px] text-[#e4e4e7] truncate">
                            {item.path || item.name}
                          </div>
                        </div>
      ...
      <div className="flex items-center justify-between mt-5 pt-4 border-t border-[#27272a]">
        <div className="text-[12px] text-[#a1a1aa]">
          ...
          {selectedSize > 0 && (
            <> · <span className="text-[#06b6d4] font-medium">{formatBytes(selectedSize)}</span> reclaimable</>
          )}
        </div>
